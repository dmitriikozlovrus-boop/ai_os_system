...
`Source`, `Todoist ID`, `Sync status`, `Sync error`, `Sync Notion hash`, `Sync Todoist hash`.
...
После свежего деплоя история конфликтов восстанавливается из `Sync Notion hash`
и `Sync Todoist hash` в самой базе `TASKS`. Если отпечатков у конкретной записи
ещё нет, текущая раскладка задачи Todoist сначала импортируется в `TASKS.Stream`.
