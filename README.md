...
Notion является источником списка доступных проектов:
...
`STREAMS`: `РАБОТА`, `БИЗНЕС`, `СЕМЬЯ`, `ЛИЧНОЕ`. Раздел определяется только полем `TASKS.Stream`;
если оно пустое, задача попадает в раздел `ПРОЧЕЕ`.
